...
import wsEXODLogo from "../../assets/images/wsexod-icon.png";
...
function Wrap() {
  ...
  const [asset, setAsset] = useState("sEXOD");
  ...
  const setMax = () => {
    if (view === 0) {
      asset === "sEXOD" ? setQuantity(sohmBalance) : setQuantity(ohmBalance);
    } else {
  ...
  const onChangeWrap = async action => {
    ...
    if (action === "wrap") {
      if (asset === "sEXOD") {
        if (ethers.utils.parseUnits(quantity, "gwei").gt(ethers.utils.parseUnits(sohmBalance, "gwei")))
          return dispatch(error("You cannot wrap more than your sEXOD balance."));
      } else {
        if (ethers.utils.parseUnits(quantity, "gwei").gt(ethers.utils.parseUnits(ohmBalance, "gwei"))) {
          return dispatch(error("You cannot wrap more than your EXOD balance."));
        }
      }
    }
    ...
    if (
      ...
      return dispatch(error("You cannot unwrap more than your wsEXOD balance."));
    }

    if (action === "wrap") finalAction = asset === "sEXOD" ? "wrapFromsOHM" : "wrapFromOHM";
    if (action === "unwrap") finalAction = asset === "sEXOD" ? "unwrapTosOHM" : "unwrapToOHM";
      ...
      <Zoom in={true} onEntered={() => setZoomed(true)}>
        <>
                  ...
                  <Grid container spacing={2} alignItems="flex-end">
                    ...
                    <Grid item xs={12} sm={4} md={4} lg={4}>
                      <div className="wrap-index">
                        ...
                        <Typography variant="h4">
                          {currentIndex ? <>{trim(currentIndex, 2)} EXOD</> : <Skeleton width="150px" />}
                        </Typography>
                      </div>
                    </Grid>
                    <Grid item xs={12} sm={4} md={4} lg={4}>
              ...
              <div className="staking-area">
                {!address ? (
                  <div className="stake-wallet-notification">
                    ...
                    <Typography variant="h6">Connect your wallet to wrap sEXOD</Typography>
                  </div>
                ) : (
                  <>
                    <Box className="stake-action-area">
                      ...
                      >
                        ...
                        >
                          <Select
                            ...
                            disableUnderline
                          >
                            <MenuItem value={"EXOD"}>EXOD</MenuItem>
                            <MenuItem value={"sEXOD"}>sEXOD</MenuItem>
                          </Select>
                        </FormControl>
                      </Box>
                      <Box
                      ...
                      >
                        {address && !isAllowanceDataLoading ? (
                          !hasAllowance(asset === "sEXOD" ? "sohm" : "ohm") && view === 0 ? (
                              ...
                              <Typography variant="body1" className="stake-note" color="textSecondary">
                                {view === 0 && (
                                  <>
                                    ...
                                    <br />
                                    Please approve Exodia to use your <b>{asset}</b> for wrapping.
                                  </>
                                )}
                              </Typography>
                            </Box>
                        ...
                        <TabPanel value={view} index={0} className="stake-tab-panel">
                          {address && hasAllowance(asset === "sEXOD" ? "sohm" : "ohm") ? (
                            <Button
                      ...
                      {quantity && (
                        <Box padding={1}>
                          <Typography variant="body2" className={classes.textHighlight}>
                            {isUnwrap
                              ? `Unwrapping ${quantity} wsEXOD will result in ${trim(convertedQuantity, 4)} ${asset}`
                              : `Wrapping ${quantity} ${asset} will result in ${trim(convertedQuantity, 4)} wsEXOD`}
                          </Typography>
                        </Box>
                      )}
                    </Box>
                    ...
                    <div className={`stake-user-data wrap-user-data`}>
                      <div className="data-row">
                        ...
                        <Typography variant="body1">
                          {isAppLoading ? (
                            ...
                            <>
                              {asset === "sEXOD" ? <>{trim(sohmBalance, 4)} sEXOD</> : <>{trim(ohmBalance, 4)} EXOD</>}
                            </>
                          )}
                        </Typography>
                      </div>
                      <div className="data-row">
                        ...
                        <Typography variant="body1">
                          {isAppLoading ? <Skeleton width="80px" /> : <>{trim(wsohmBalance, 4)} wsEXOD</>}
                        </Typography>
                      </div>
                    </div>
                  </>
          ...
          {/* <Paper className={`ohm-card`}>
            ...
            <WrapFarm>
              <FarmPair>
                <img src={wsEXODLogo} alt="wsEXOD logo" />
                <img src={BeetsLogo} alt="Beethovenx logo" />
                <Pair>wsEXOD-BEETS</Pair>
              </FarmPair>
              <Button
                variant="outlined"
